...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0 fw-bold">Edit User: <?= e($user['username'] ?? '') ?></h4>
        ...
        <form method="POST" action="<?= url('/admin/users/update/' . $user['id']) ?>">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control" name="display_name" value="<?= e($user['display_name'] ?? '') ?>" required>
                </div>
                <div class="col-md-6">
                    ...
                    <select class="form-select" name="role">
                        ...
                        @if(isset($roles))
                            @foreach($roles as $role)
                                <option value="<?= $role['id'] ?>" <?= in_array($role['id'], $userRoleIds ?? []) ? 'selected' : '' ?>><?= e($role['name']) ?></option>
                            @endforeach
                        @endif
                    </select>
                </div>
                <div class="col-md-6">
                    ...
                    <select class="form-select" name="is_admin">
                        <option value="0" <?= empty($user['is_admin']) ? 'selected' : '' ?>>No</option>
                        <option value="1" <?= !empty($user['is_admin']) ? 'selected' : '' ?>>Yes</option>
                    </select>
                </div>
                <div class="col-12">
                    <label class="form-label fw-semibold">Description</label>
                    <textarea class="form-control" name="description" rows="3"><?= e($user['description'] ?? '') ?></textarea>
                </div>
                <div class="col-12">
